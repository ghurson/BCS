...
class Kopa_Settings_Sidebar_Manager extends Kopa_Settings_Page {
	...
	public function __construct() {
		...
		$this->label = __( 'Sidebar Manager', kopa_get_domain() );
	...
	public function get_page_settings() {
		...
		return apply_filters( 'kopa_sidebar_manager_settings', array(
			array(
				'title'   => __( 'Sidebar Manager', kopa_get_domain() ),
				'type' 	  => 'title',
				'id' 	  => 'sidebar-title',
			),
			array(
				'title'       => 'Sidebar Manager',
				'description' => __( 'Add as many sidebars (widget areas) as you need. Creating sidebars (widget areas) is unlimited.', kopa_get_domain() ),
				'id'          => 'sidebar-manager',
