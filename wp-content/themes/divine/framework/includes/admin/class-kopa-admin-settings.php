...
class Kopa_Admin_Settings {
	...
	public static function save() {
		...
		if ( empty( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'kopa-settings' ) ) {
    		die( __( 'Action failed. Please refresh the page and retry.', kopa_get_domain() ) );
		}
		...
		if ( isset( $_POST['kopa_reset'] ) ) {
			...
			self::add_message( __( 'Default options restored.', kopa_get_domain() ) );
		}
		// save current tab settings
		else {
	   		do_action( 'kopa_settings_save_' . $kopa_current_tab );
			self::add_message( __( 'Your settings have been saved.', kopa_get_domain() ) );
		}
