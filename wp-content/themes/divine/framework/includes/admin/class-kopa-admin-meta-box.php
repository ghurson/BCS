        	...
        	switch( $value['type'] ) {
				...
				case 'upload':
					...
					if ( function_exists( 'wp_enqueue_media' ) ) {
						if ( $option_value == '' ) {
							$output .= '<a style="margin-top: 3px" class="kopa_upload_button button">'.esc_html__( 'Upload', kopa_get_domain() ).'</a>';
						} else {
							$output .= '<a style="margin-top: 3px" class="kopa_remove_file button">'.esc_html__( 'Remove', kopa_get_domain() ).'</a>';
						}
					} else {
						$output .= '<small class="kopa_upload_notice">'.esc_html__( 'Upgrade your version of WordPress for full media support.', kopa_get_domain() ).'</small>';
					}
					$output .= $option_wrap_end;
					...
					if ( $option_value ) {
						$remove = '<a class="button kopa_remove_image">'.esc_html__( 'Remove', kopa_get_domain() ).'</a>';
