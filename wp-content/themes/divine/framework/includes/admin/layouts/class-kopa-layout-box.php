...
class Kopa_Layout_Box {
	...
	public function output( $use_custom_layout, $option_value, $value ) {
		...
		$output .= '<label><input class="kopa_use_custom_layout" type="checkbox" name="_use_custom_layout" value="'.esc_attr( $settings['layout'] ).'" '.checked( $settings['layout'], $use_custom_layout, false ).'> '.__( 'Check if you would like to use custom setting', kopa_get_domain() ).'</label>';
		...
		$output .= '<h4 class="kopa_heading">'.__( 'Select layout', kopa_get_domain() ).'</h4>';
		...
		foreach ( $value['layouts'] as $layout_id => $layout_args ) {
			...
			foreach ( $layout_args['positions'] as $position_index => $position ) {
				...
				$output .= '<option value="">'.__( '&mdash;Select sidebar&mdash;', kopa_get_domain() ).'</option>';
		...
		if ( $settings['taxonomy'] ) {
			echo '<tr>';
				echo '<th scope="row" valign="top">';
					echo __( 'Custom Layout', kopa_get_domain() );
				echo '</th>';
				echo '<td>';
		}
