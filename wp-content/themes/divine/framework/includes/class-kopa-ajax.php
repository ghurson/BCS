...
class Kopa_Ajax {
	...
	public function remove_sidebar() {
		...
		if ( isset( $sidebars_widgets[ $sidebar_id ] ) && ! empty( $sidebars_widgets[ $sidebar_id ] ) ) {
			$warnings[] = __( 'This sidebar is currently containing widgets.', kopa_get_domain() );
		}
					...
					if ( ! empty( $option_value['sidebars'] ) && is_array( $option_value['sidebars'] ) ) {
						foreach ( $option_value['sidebars'] as $layout_id => $sidebars ) {
							...
							if ( is_array( $sidebars ) && in_array( $sidebar_id, $sidebars ) ) {
								...
								$errors[] = "&nbsp;&nbsp;&nbsp;&nbsp;" . sprintf( __( '%1$s of %2$s.', kopa_get_domain() ), $layout_name, $page_title );
							}
						}
					}
				}
		...
		// warning messages
		// if ( ! empty( $warnings ) ) {
		// 	$warnings[] = __( 'Delete it will also delete all widgets are inside it.', kopa_get_domain() );
		// }
		...
		if ( ! empty( $errors ) ) {
			$allow_delete = false;
			array_unshift( $errors, '<strong>' . __( 'This sidebar is being used in:', kopa_get_domain() ) . '</strong>' );
			$errors[] = '<strong>' . __('You cannot delete this sidebar.', kopa_get_domain()) . '</strong>';
		}
