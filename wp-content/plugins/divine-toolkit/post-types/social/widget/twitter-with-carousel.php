...
add_action( 'widgets_init', array('DVT_Widget_Twitter_Width_Carousel', 'register_widget'));

class DVT_Widget_Twitter_Width_Carousel extends Kopa_Widget {

    public $kpb_group = 'social';
    
    public static function register_widget(){
        register_widget('DVT_Widget_Twitter_Width_Carousel');
    }

	public function __construct() {
		$this->widget_cssclass    = 'kopa-twitter-widget kopa-twitter-widget-carousel';
		$this->widget_description = __( 'Display list tweets from twitter.com with carousel effect.', 'divine-toolkit' );
		$this->widget_id          = 'kopa-widget-latest-tweets-carousel';
		$this->widget_name        = __( 'Latest Tweets (carousel)', 'divine-toolkit' );
		$this->settings 		  = array(
			'title'  => array(         
				'type'  => 'text',
				'std'   => '',
            ...
            'tweets_per_slide'  => array(
                'type'  => 'text',
                'std'   => 3,
                'label' => __( 'Tweets per slide', 'divine-toolkit' )
            ),  	
            'consumer_key'  => array(
                'type'  => 'text',
                'std'   => '',
	...
	public function widget( $args, $instance ) {
        ...
        if ($screen_name && $count && $tweets_per_slide && $consumer_key && 
            $consumer_secret && $oauth_access_token && $oauth_access_token_secret) {

            ?>
            <input type="hidden" class="tweets-data" name="security" value="<?php echo wp_create_nonce('dvt_load_tweets_carousel'); ?>" autocomplete="off">
...
if (!function_exists('dvt_load_tweets_carousel')) {
    add_action('wp_ajax_dvt_load_tweets_carousel', 'dvt_load_tweets_carousel');
    add_action('wp_ajax_nopriv_dvt_load_tweets_carousel', 'dvt_load_tweets_carousel');

    function dvt_load_tweets_carousel() {
        check_ajax_referer('dvt_load_tweets_carousel', 'security');
                 ...
                 if ((isset($data["error"]) || !empty($data["error"])) || 
                    ...
                    if (!empty($data)){
                        ...
                        foreach ($data as $items){
                            if ($loop_index == $tweets_per_slide) {
                                $data_index++;
                                $loop_index = 1;
                            } else {
                                $loop_index++;
                            ...
                            <?php
                            $tweets[$data_index][$loop_index] = ob_get_clean();
                        }                        
